...
echo Starting Home Proxy...
start "Clipt Home Proxy" /min python "%~dp0home_proxy.py"

:: Wait for Flask to write its port to the temp file
timeout /t 4 /nobreak >nul

:: Read the actual port Flask chose (dynamic — avoids port conflicts)
set PROXY_PORT=5050
if exist "%TEMP%\clipt_proxy_port.txt" (
    set /p PROXY_PORT=<"%TEMP%\clipt_proxy_port.txt"
)
echo Detected proxy port: %PROXY_PORT%

:: Quick health check on the actual port
curl -s http://localhost:%PROXY_PORT%/health >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Home proxy failed to start on port %PROXY_PORT%
    echo Trying port 5050...
    set PROXY_PORT=5050
    curl -s http://localhost:5050/health >nul 2>&1
    if %ERRORLEVEL% NEQ 0 (
        echo ERROR: Home proxy not responding on any port.
        pause
        exit /b 1
    )
)
echo Home proxy is running on port %PROXY_PORT%
echo.
